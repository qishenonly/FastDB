...
func TestNewFastDB(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "fastdb")
	opts.DirPath = dir
...
func TestDB_Put(t *testing.T) {
	...
	assert.Equal(t, ers.ErrKeyIsEmpty, err)
...
func TestDB_Delete(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "fastdb-delete")
	opts.DirPath = dir
	opts.DataFileSize = 64 * 1024 * 1024
	db, err := NewFastDB(opts)
	defer destroyDB(db)
...
func TestDB_GetListKeys(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "fastdb-ListKey")
	opts.DirPath = dir
	opts.DataFileSize = 64 * 1024 * 1024
	db, err := NewFastDB(opts)
	defer destroyDB(db)
...
func TestDB_Fold(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "fastdb-fold")
	opts.DirPath = dir
	opts.DataFileSize = 64 * 1024 * 1024
	db, err := NewFastDB(opts)
	defer destroyDB(db)
...
func TestDB_Close(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "fastdb-close")
	opts.DirPath = dir
	opts.DataFileSize = 64 * 1024 * 1024
	db, err := NewFastDB(opts)
	assert.Nil(t, err)
