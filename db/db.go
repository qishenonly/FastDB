...
// NewFastDB 打开 bitcask 存储引擎实例
func NewFastDB(options Options) (*DB, error) {
	// 对用户传入的配置项进行校验
...
func (db *DB) Put(key []byte, value []byte) error {
	...
	if ok := db.index.Put(key, pos); !ok {
		return ers.ErrIndexUpdateFailed
	}
...
func (db *DB) getValueByPosition(logRecordPst *data.LogRecordPst) ([]byte, error) {
	...
	//数据文件为空
	if dataFile == nil {
		return nil, ers.ErrDataFailNotFound
	}
...
func checkOptions(options Options) error {
	if options.DirPath == "" {
		return ers.ErrOptionDirPathIsEmpty
	}
	if options.DataFileSize <= 0 {
		return ers.ErrOptionDataFileSizeNotPositive
	}
	return nil
}
...
func (db *DB) loadDataFiles() error {
	...
	for _, entry := range dirEntry {
		if strings.HasSuffix(entry.Name(), data.DataFileSuffix) {
			...
			//数据目录可能损坏
			if err != nil {
				return ers.ErrDataDirectoryCorrupted
			}
...
func (db *DB) loadIndexFromDataFiles() error {
	...
	updataIndex := func(key []byte, typ data.LogRecrdType, pst *data.LogRecordPst) {
		...
		if !ok {
			panic(ers.ErrIndexUpdateFailed)
		}
	}
